...
from sc2reader.objects import Player, Message, Color, Observer, Team, Packet, Map
    ...
    if filter(lambda t: t.result == "Unknown", replay.teams):
        ...
        for event in replay.events_by_type['PlayerLeave']:
            ...
            if event.pid in replay.player:
                ...
                # If the recorder is playing, and the recorder leaves, then
                # the recorder's team lost.  This is not always true, but it's
                # a very good heuristic.  We can correct this in the future by
                # scraping a source like battle.net or sc2ranks.
                #
                # The only time this won't work is when the recorder quits a game,
                # and their game goes on to win it without him.  It's worth it to 
                # be wrong in these rare cases in order to give the correct result
                # in the far more common case, which is someone leaving a 2v2, 3v3
                # or 4v4 with a gg and a quit, before their teammates have left.
                #
                # TODO correct this using battle.net or sc2ranks.
                #
                if replay.recorder is not None:
                    if event.pid == replay.recorder.pid:
                        replay.player[event.pid].team.result = "Loss"
                        if len(replay.team) == 2:
                            losingteam = replay.player[event.pid].team
                            replay.team[3 - losingteam.number].result = "Win"
